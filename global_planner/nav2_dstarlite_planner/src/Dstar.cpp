...
Dstar::Dstar() {
  ...
  maxSteps = 500000;  // node expansions before we give up
  C1       = 255;      // cost of an unseen cell
  

}
Dstar::Dstar(nav2_costmap_2d::Costmap2D * msg) {

  maxSteps = INT8_MAX;  // node expansions before we give up
  C1       = 255;      // cost of an unseen cell
  costMapCallback(msg);
...
bool Dstar::occupied(state v) {
  ...
  // v.x = v.y;
  // v.y = temp;
  ds_ch::iterator cur = cellHash.find(v);


  if (cur == cellHash.end()) {
    //std::cout << "OCC FALSE CUZ NOT FOUND" << v.x << " " << v.y << " " << v.k.first << " " << v.k.second << " " << std::endl;
    return false;
  }

 if (cur->second.cost > 128 &&   cur->second.cost  < 255){
  //std::cout << "OCC TRUE CUZ HIGH" << std::endl
  return true;
 }
 else{
////std::cout << "OCC FALSE CUZ LOW " <<  cur->second.cost <<  std::endl;
...
void Dstar::printCell(int i, int j) {
            state u;
            u.x = i;
            u.y = j;
           ...
           //std::cout<< u.k.second << " ";
            ...
            double d;
              if (cur == cellHash.end()) {
              ////std::cout << "OCC FALSE CUZ NOT FOUND" << std::endl;
                d = -2.0;
              }
              else{
                //std::cout << "OCC TRUE CUZ HIGH" << std::endl
...
int Dstar::computeShortestPath() {
 ...
 //std::cout << "compute Shortest Path end"<< std::endl;
...
void Dstar::updateCell(int x, int y, double val) {
  ...
  //std::cout << "updating cell " << x << " " << y << " " << u.k.first << " " << u.k.second  << std::endl;
 ...
 */
void Dstar::getSucc(state u,list<state> &s, ds_set& myset) {
...
bool Dstar::replan() {
 //std::cout << "planing started" << std::endl;
  if(this->hasinit == false){
      this->init(s_start.x,s_start.y,s_goal.x, s_goal.y);
      this->hasinit = true;
      return false;
  }
  ...
  std::ofstream my;
    ...
    int c = 0;
  my << "cur goal start" << cur.x << " " << cur.y << " " << s_goal.x << " " << s_goal.y << std::endl;
  myset.insert(ur);
  while(cur != s_goal) {
    c++;
    if(c == counter)
     ...
     my << "cur goal loop" << cur.x << " " << cur.y << " " << s_goal.x << " " << s_goal.y << std::endl;
    ...
    for (i=n.begin(); i!=n.end(); i++) {
     ...
     //not needed nothing in n should be occupied if (occupied(*i)) continue;
 ...
 //std::cout << "cur goal end " << cur.x << " " << cur.y << " " << s_goal.x << " " << s_goal.y << std::endl;

  path.push_back(s_goal);
 //std::cout << "planing ended" << std::endl;
 //std::cout << "plan has  "<< path.size() << " values" << std::endl;

  return true;
}

void Dstar::costMapCallback(nav2_costmap_2d::Costmap2D* msg){
   

    int costmap_width = msg->getSizeInCellsX();
    int costmap_height = msg->getSizeInCellsY();
   //std::cout <<"callback " << costmap_height << " " << costmap_width << std::endl;
 

    std::ofstream myfile4;
    myfile4.open ("map35.txt");

      for(int i = 0; i < costmap_height; i++){
        for(int j = 0; j < costmap_width; j++){
          ////std::cout << msg->getCost(j,i) << " " <<  msg->getCost(j,i) << std::endl;
            // int x = (int)msg->getCost(j,i) ;
            // if( x == 255)
            //   updateCell(i,j, -1);
            // else
              updateCell(i,j, (int)msg->getCost(j,i));
             // myfile4 << "aupdating " << i << " " << j << std::endl;
        }
    ...
    std::ofstream ofs;
    ofs.open("map3.txt", std::ofstream::out | std::ofstream::trunc);
    ofs.close();
    ...
    myfile.open ("map3.txt");
     ...
     myfile.close();
         ...
         std::ofstream ofs2;
    ofs2.open("map4.txt", std::ofstream::out | std::ofstream::trunc);
    ofs2.close();

    std::ofstream myfile2;
    myfile2.open ("map4.txt");
