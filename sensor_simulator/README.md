...
- Yaw is reported relative to **true north**
...
## Broadcasted TF
- `map` → `base_link_ground_truth` - Noiseless true robot pose, broadcast every high-rate update
...
| `base_frame_id` | `str` | `base_link` | TF frame ID for the robot base frame (used by EKF / localization) |
| `ground_truth_base_frame_id` | `str` | `base_link_ground_truth` | TF frame for the true robot pose. Broadcaster publishes `map` → this frame; `odom/ground_truth` uses it as `child_frame_id` |
