...
rpastar::rpastar(std::pair<int,int> start_state_in, std::pair<int,int> target_state_in, nav2_costmap_2d::Costmap2D * msg) : start_state(start_state_in), target_state(target_state_in)
{
    //these are in the a* old implementation but don't think we need them as they are called in subscribe in listener.cpp
    // gpsCall();
    std::cout <<"rpastar ctor init\n";
    

    costMapCallback(msg); 
    std::cout <<"rpastar ctor finsih\n";
...
void rpastar::search()
{
    ...
    while (!U.empty())
    {
        ...
        processNode(i-1, j+1, parent_ptr); // north-east
        processNode(i+1, j+1, parent_ptr);// south-east
        processNode(i-1, j-1, parent_ptr); // north-west
        processNode(i+1, j+1, parent_ptr); // south-west
    ...
    if(path_found){
        std::cout <<"Search concluded because path was found\n";
    }
    else{
        std::cout <<"Search concluded because U was empty\n";
    }
...
void rpastar::processNode(int row, int col, Node *parent)
{
    ...
    if (cost_map[row][col] > 190 && cost_map[row][col] < 254)
    {
        return;
    }
...
void rpastar::costMapCallback(nav2_costmap_2d::Costmap2D* msg)
{
    std::cout <<"costMapCallback init\n";
...
//initializing graph of nodes
    ...
    for(int i = 0; i < costmap_height; i++){
        for(int j = 0; j < costmap_width; j++){
            cost_map[i][j] = msg->getCost(j,i) ;
            // if(cost_map[i][j] != 207 )
            //     std::cout << "!!!!!!!" << cost_map[i][j] << "!!!!!!!\n";
    ...
    std::cout << "finsished costMapCallback\n";
